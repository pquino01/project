...
public class ShoppingCart implements Serializable {
     ...
     */
    public synchronized void addItem(Product product, short quantityProducts) {
        ...
        for (ShoppingCartItem scItem : items) {
            ...
            if (scItem.getProduct().getId() == product.getId()) {
                ...
                newItem = false;
                scItem.incrementQuantity(quantityProducts);
            }
        }

        if (newItem) {
            ShoppingCartItem scItem = new ShoppingCartItem(product);
            scItem.setQuantity(quantityProducts);
            items.add(scItem);
        }
    }
     ...
     * @see ShoppingCartItem
     */
    public synchronized void update(Product product, short qty) {
     ...
     * @see ShoppingCartItem
     */
    public synchronized double getTotalCartPrice() {
